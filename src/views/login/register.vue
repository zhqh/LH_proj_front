<template>
  <div class="login">
    <div class="language">
      <span :class="{ selected: currentLanguage == 'zh-cn' }" @click="changeLang('zh-cn')"> 中文 </span>
      <span :class="{ selected: currentLanguage == 'en-us' }" @click="changeLang('en-us')"> English </span>
    </div>
    ...
    <div class="base-info">
      <nut-form ref="ruleForm" :model-value="baseInfo">
        <nut-form-item required prop="username" :rules="[{ required: true, message: $t('register.empty_name') }]">
          <My />
          <input v-model="baseInfo.username" class="nut-input-text" :placeholder="$t('register.input_name')" type="text" />
        </nut-form-item>
        <nut-form-item required prop="password" :rules="[{ required: true, message: $t('register.empty_password') }]">
          <Order />
          <input v-model="baseInfo.password" class="nut-input-text" :placeholder="$t('register.input_password')" type="password" />
        </nut-form-item>
        <nut-form-item required prop="email" :rules="[{ required: true, message: $t('register.empty_email') }]">
          <My />
          <input v-model="baseInfo.email" class="nut-input-text" v-bind:placeholder="$t('register.input_email')" type="text" />
        </nut-form-item>
        <nut-form-item required prop="emailCaptcha" :rules="[{ required: true, message: $t('common.empty_captcha') }]">
          <Order />
          <input v-model="baseInfo.emailCaptcha" class="nut-input-text" :placeholder="$t('common.input_captcha')" type="text" />
          <nut-button type="primary" size="small" shape="square" @click="sendEmailCaptcha">{{ $t('common.send_captcha') }}</nut-button>
        </nut-form-item>
        <nut-form-item required prop="invitedCode" :rules="[{ required: true, message: $t('register.empty_invitedCode') }]">
          <My />
          <input v-model="baseInfo.invitedCode" class="nut-input-text" :placeholder="$t('register.input_invitedCode')" type="text" />
        </nut-form-item>
        <div class="button-align-center">
          <nut-button block type="info" @click="register" :disabled="saving"> {{ $t('register.register') }} </nut-button>
        </div>
        <div class="bottom">
