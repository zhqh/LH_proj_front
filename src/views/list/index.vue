<template>
  <div class="header">
    <div class="total-floatPL">{{ $t('list.floatPL') }}(USDT)<span
        :class="{ red: strategies.floatPL > 0, green: strategies.floatPL < 0 }"><b>{{ strategies.floatPL
        }}&nbsp;</b>
      </span>
      <span class="his-stratergy">
        <Refresh @click="refresh" />
        <router-link to="/hisst"><span>{{ $t('list.hisstratety') }}</span></router-link>
      </span>
    </div>
    ...
    <div class="condition">
      ...
      <span @change="reloadStrategy"><input type="checkbox" v-model="showBacktest">
        <label>{{ $t('list.backtestOnly') }}</label></span>
    </div>
    <nut-infinite-loading v-model="infinityValue" load-txt="Loading..." :load-more-txt="$t('common.nomore')"
      :has-more="hasMore" @load-more="getStrategy">
      <div v-for="item in strategies.data" :key="item.id"
        :class="{ running: item.status == 'RUNNING', pause: item.status == 'PAUSE' }">
        <div class="stratergy-head">
          <div class="stratergy-item-left">
            <div><span class="instId">{{ item.instId }}</span><span class="stratergy-type">{{
              tradeUtil.getNameDesc(item.name, globalProperties)
            }}</span>&nbsp;<span></span></div>
            <div class="status">
              <IconFont name="check-normal"></IconFont>
              <b @click="showPauseRemark(item.id)"><span :class="{ underline: item.status == 'PAUSE' }">{{
                item.status == 'RUNNING' ? $t('list.running') : $t('list.pausing')
              }}</span>{{ item.backtest ? $t('list.backtest')
  : '' }}</b><span class="seperate">{{ $t('list.strategyFloatFL') }}：</span><span
                :class="{ red: item.floatPL > 0, green: item.floatPL < 0 }">{{ item.floatPL && item.floatPL.toFixed(2)
                }}{{
  item.unit }}</span>
            </div>
          </div>
          <div class="stratergy-item-right">
            <nut-button v-if="item.backtest == false" block type="info" size="mini" @click="process(item.id)"
              color="#7232dd" plain>{{
                item.status == 'RUNNING' ? $t('list.pause') : $t('list.proceed')
              }}</nut-button>
            <nut-button v-if="item.backtest == false" block type="info" size="mini" @click="stop(item.id)" color="#7232dd"
              plain>{{ $t('list.stop') }}</nut-button>
          </div>
        </div>
        <div class="stratergy-item">
          <div class="stratergy-item-left">{{ $t('list.todayProfit') }}<span class="digit">{{ item.todayProfit.toFixed(2)
          }}{{ item.unit }}</span>
          </div>
          <div class="stratergy-item-right">{{ $t('list.accumulatedProfit') }}<span class="digit">{{
            item.accumulatedProfit.toFixed(2) }}{{ item.unit
  }}({{
  (item.accumulatedProfitRate * 100).toFixed(0) + '%' }})</span>
          </div>
        </div>
        <div class="stratergy-item">
          <div class="stratergy-item-left">{{ $t('list.openQuantity') }}<span class="digit">{{
            item.openQuantity }}{{ item.unit }}</span></div>
          <div class="stratergy-item-right">{{ $t('list.openCount') }}<span class="digit">{{ item.openCount }}单</span>
          </div>
        </div>
        <div class="stratergy-item">
          <div class="stratergy-item-left">{{ $t('list.closeCount') }}<span class="digit">{{ item.closeCount }}单</span>
          </div>
          <div class="stratergy-item-right">{{ $t('list.maxLoss') }}<span class="digit">{{ (Math.abs(item.maxLoss) *
            100).toFixed(0)
          }}%</span>
          </div>
        </div>
        <div class="stratergy-item">
          <div class="stratergy-item-left">{{ $t('list.startTime') }}<span class="digit">{{ item.startTime }}</span></div>
          <div class="stratergy-item-right">{{ $t('list.duration') }}<span class="digit">{{ item.duration }}天</span></div>
        </div>
        <div class="stratergy-item">
          <div class="stratergy-item-left">{{ $t('list.liqPx') }}<span class="digit">{{ item.liqPx || '--' }}</span></div>
        </div>
        <div class="operation">
          <a @click="showOrderPlan(item.id)"><span class="seperate operation-font-color">{{ $t('list.orderPlan')
          }}</span></a>
          <router-link :to="{ path: `/stparam/${item.strategyParamId}` }"><span class="seperate operation-font-color">{{
            $t('list.strategyParam') }}</span></router-link>
          <router-link :to="{ path: `/sthold/${item.id}` }"><span class="seperate operation-font-color">{{
            $t('list.holdOrder') }}</span></router-link>
          <router-link :to="{ path: `/sthishold/${item.id}` }"><span class="seperate operation-font-color">{{
            $t('list.hisOrder') }}</span></router-link>
        </div>
...
onActivated(() => {
  reloadStrategy();
  getFloatPL();
  interval.value = setInterval(() => { getFloatPL(); }, 5 * 1000);
});
...
const getFloatPL = async () => {
  const r = await useStrategyApi().getTotalFloatFL();
  if (r.code == 0) {
