<template>
  <div class="header">
    <div class="condition">
      ...
      <span @change="reloadStrategy"><input type="checkbox" v-model="showBacktest" /> <label>只显示回测</label></span>
    </div>
    ...
    >
      <div v-for="item in strategies.data" :key="item.id" class="history" @click="detail(item.id)">
        ...
        <div class="stratergy-item">
          <div class="stratergy-item-left"
            >平仓单量<span class="digit">{{ item.closeCount }}</span></div
          >
          <div class="stratergy-item-right"
            >最大回撤<span class="digit">{{ (Math.abs(item.maxLoss) * 100).toFixed(0) + '%' }}</span></div
          >
        </div>
        <div class="stratergy-item">
          <div class="stratergy-item-left"
