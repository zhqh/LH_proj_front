<template>
  <div class="header-stcreate">
    ...
    <div class="course-type">
      ...
      <nut-row v-if="baseStrategyParam.param.backtest != 'true'">
        <nut-col :span="5">
        ...
        <nut-col :span="2">
          <nut-input placeholder="" v-model="baseStrategyParam.param.lever" type="number" />
        </nut-col>
        <nut-col :span="5">
          <nut-button block type="info" size="mini" @click="resetLever" color="#7232dd" plain style="display: none;">{{
            $t('create_strategy.modify_lever') }}</nut-button>
        </nut-col>
      </nut-row>
    </div>
    <keep-alive>
    ...
    <div class="course-type">
      ...
      <nut-row v-show="baseStrategyParam.param.triggerCondition == 'price'">
        ...
        <nut-col :span="19">
          <div>
            <nut-input :placeholder="$t('create_strategy.input_triggerPrice')"
              v-model="baseStrategyParam.param.triggerPrice" />
          </div>
        </nut-col>
      </nut-row>
      ...
      <nut-row>
        ...
        <nut-col :span="19">
          <div class="close-strategy">
            ...
            <div>
              <nut-input placeholder="" v-model="baseStrategyParam.param.closeProfitRate" input-align="right" />
            </div>
            <span>%</span>
            <span class="loss-label">{{ $t('create_strategy.loss') }}</span>
            <div>
              <nut-input placeholder="" v-model="baseStrategyParam.param.closeLossRate" input-align="right" />
            </div>
            <span>%</span>
          </div>
        </nut-col>
    ...
    <div class="course-type">
      <div class="button-align-center">
        <nut-button block type="info" @click="onKeyStart" :disabled="disableOneKeyStart">{{
          baseStrategyParam.param.backtest == 'true' ? $t('create_strategy.start_backtest') :
          $t('create_strategy.start_strategy')
        }}</nut-button>
    ...
    <div>
      <nut-popup position="bottom" v-model:visible="nameShow">
        <nut-picker v-model="name" :columns="nameColumns" :title="$t('create_strategy.select_strategy_name')"
          @confirm="namePopupConfirm" @cancel="nameShow = false" />
      </nut-popup>
      <nut-popup position="bottom" v-model:visible="tradeTypeShow">
        <nut-picker v-model="tradeType" :columns="tradeTypeColumns" :title="$t('create_strategy.select_trade_type')"
          @confirm="tradeTypePopupConfirm" @cancel="tradeTypeShow = false" />
      </nut-popup>
      <nut-popup position="bottom" v-model:visible="instIdShow">
        ...
        <nut-picker v-model="instIdSearch.located" :columns="instIdColumns" :title="$t('create_strategy.select_instId')"
          @confirm="instIdPopupConfirm" @cancel="instIdShow = false" />
      </nut-popup>
      <nut-popup position="bottom" v-model:visible="triggerConditionShow">
        <nut-picker v-model="triggerCondition" :columns="triggerConditionColumns"
          :title="$t('create_strategy.select_trigger_condition')" @confirm="triggerConditionPopupConfirm"
          @cancel="triggerConditionShow = false" />
      </nut-popup>
      <nut-popup position="bottom" v-model:visible="queryDate.startShow">
        <nut-date-picker type="year-month" v-model="queryDate.startDate" :min-date="queryDate.startMinDate"
          :max-date="queryDate.startMaxDate" @confirm="startPopupConfirm" :is-show-chinese="true"
          :threeDimensional="false" />
      </nut-popup>
      <nut-popup position="bottom" v-model:visible="queryDate.endShow">
        <nut-date-picker type="year-month" v-model="queryDate.endDate" :min-date="queryDate.endMinDate"
          :max-date="queryDate.endMaxDate" @confirm="endPopupConfirm" :is-show-chinese="true" :threeDimensional="false" />
      </nut-popup>
    </div>
  </div>
</template>
...
let currentStrategy = strategyMap.SimpleGrid;
...
const queryDate = reactive({
  startShow: false,
  startPopupDesc: getRecentYearMonthString(3),
...
// const name = ref();
const nameDesc = ref(globalProperties.$t('strategy_name.SimpleGrid'));
...
const tradeTypeChanged = async () => {
  ...
  instIdColumns.value = instruments.map(e => { return { text: e.instId, value: e.instId } });
  instIdDesc.value = '';
...
const instIdPopupConfirm = async ({ selectedValue, selectedOptions }) => {
  ...
  getLowestOpenQuantity(baseStrategyParam.param.instId, baseStrategyParam.param.tradeType).then(e => baseStrategyParam.context.lowestOpenQuantity = e);
};
...
onActivated(async () => {
  ...
  if (baseStrategyParam.param.backtest == 'true') {
    baseStrategyParam.param.exchange = 'simulate';
  }
  getApiKey().then(e => {
  ...
  loadInstruments(tradeType, null).then(e => {
    instIdColumns.value = e.map(e => { return { text: e.instId, value: e.instId } });
  });
});

const disableOneKeyStart = ref(true);
watch(() => { return [baseStrategyParam.param.instId, baseStrategyParam.context.apiKey] }, (newValues, oldValues) => {
