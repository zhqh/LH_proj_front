<template>
    ...
    <nut-row>
      ...
      <nut-col :span="7">
        <div class="dropdown-contain">
          <nut-input placeholder="" v-model="customStrategyParam.param.low_price" />
        </div>
      </nut-col>
      <nut-col :span="3">
      ...
      <nut-col :span="9">
        <nut-input placeholder="" v-model="customStrategyParam.param.high_price" />
      </nut-col>
    </nut-row>
    <nut-divider :style="styleObject" />
  </div>
  <h3 class="basic-title">{{ $t('list.orderPlan') }}<span>{{ $t('create_strategy.lowestOpenQuantity') }}{{
  ...
  <div class="course-type">
    <nut-row>
      ...
      <nut-col :span="7">
        <div class="dropdown-contain target-profit">
          <nut-input placeholder="" v-model="customStrategyParam.param.batch_profit_rate" input-align="right" />%
        </div>
      </nut-col>
      <nut-col :span="5">
      ...
      <nut-col :span="7">
        <div class="dropdown-contain target-profit">
          <nut-input placeholder="" v-model="customStrategyParam.param.batch_loss_rate" input-align="right" />%
        </div>
      </nut-col>
    </nut-row>
    <nut-divider :style="{ color: 'red', borderColor: '#1989fa', padding: '0 16px' }" content-position="left">{{
      $t('create_strategy.long_position') }}</nut-divider>
    <nut-row>
      <div class="grid-head">
        <nut-col :span="3">
    ...
    <nut-row v-for="(item, index) in customStrategyParam.param.long_grid_lines" :key="item.id">
      <div class="grid-line">
        ...
        <nut-col :span="6">
          <nut-input placeholder="" v-model="item.quantity" input-align="left" />
        </nut-col>
        <nut-col :span="5">
          <nut-input placeholder="" v-model="item.addition_gap" input-align="left" />
        </nut-col>
        <nut-col :span="3">
          <nut-input placeholder="" v-model="item.target_profit_rate" input-align="left" />
        </nut-col>
        <nut-col :span="3">
          <nut-input placeholder="" v-model="item.adjust_rate" input-align="left" />
        </nut-col>
        <nut-col :span="4">
          <nut-input placeholder="" v-model="item.loss_condition_rate" input-align=" left" /><span
            @click="deleteLongRow">-</span>
        </nut-col>
      </div>
    </nut-row>
    <br />
    <nut-divider :style="{ color: 'green', borderColor: '#1989fa', padding: '0 16px' }" content-position="left">{{
      $t('create_strategy.short_position') }}</nut-divider>
    <nut-row>
      <div class="grid-head">
        <nut-col :span="3">
          <div class="flex-content">{{ $t('common.num') }}</div>
  ...
  <div>
    <nut-popup position="bottom" v-model:visible="profitModeShow">
      <nut-picker v-model="profitMode" :columns="profitModeColumns" title="请选择盈利方式" @confirm="profitModePopupConfirm"
...
const getCustomParam = async (param, context) => {
  ...
  return { ...customStrategyParam.param, position, addition_count, long_grid_lines, short_grid_lines, batch_profit_rate, batch_loss_rate };
}
...
const verify = async (baseStrategyParam) => {
  ...
  let errorRow = customStrategyParam.param.long_grid_lines.some(e => !e.quantity || !e.addition_gap || !e.target_profit_rate)
  ...
  errorRow = customStrategyParam.param.long_grid_lines.some(e => e.quantity * baseStrategyParam.param.lever < baseStrategyParam.context.lowestOpenQuantity)
    || customStrategyParam.param.short_grid_lines.some(e => e.quantity * baseStrategyParam.param.lever < baseStrategyParam.context.lowestOpenQuantity);
  if (errorRow) {
    showToast(`${globalProperties.$t('create_strategy.lowestOpenQuantity')}<${baseStrategyParam.context.lowestOpenQuantity}${baseStrategyParam.param.unit}`);
    return false;
  }
...
const addLongRow = async () => {
  customStrategyParam.param.long_grid_lines.push({ id: new Date().valueOf(), quantity: 100, addition_gap: 8, target_profit_rate: 5, adjust_rate: 1, loss_condition_rate: '' });
}
...
const addShortRow = async () => {
  customStrategyParam.param.short_grid_lines.push({ id: new Date().valueOf(), quantity: 100, addition_gap: 8, target_profit_rate: 5, adjust_rate: 1, loss_condition_rate: '' });
}
