...
const verify = async (baseStrategyParam) => {
  ...
  errorRow = customStrategyParam.param.grid_lines.some(e => e.quantity * baseStrategyParam.param.lever < baseStrategyParam.context.lowestOpenQuantity);
  if (errorRow) {
...
const addRow = async () => {
  customStrategyParam.param.grid_lines.push({ id: new Date().valueOf(), quantity: 100, addition_gap: 8, target_profit_rate: 5, adjust_rate: 1, loss_condition_rate: '' });
}
