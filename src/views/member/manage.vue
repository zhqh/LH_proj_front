<template>
  <div>
    <div class="course-type">
      ...
      <h1>
        <nut-button block type="info" @click="downloadHistoryCandles"> 下载历史行情数据 </nut-button>
      </h1>
      <h1>
        <nut-button block type="info"> 同步持仓数据到当前服务器节点 </nut-button>
      </h1>
    </div>
  </div>
</template>
...
<script setup name="DemoPage" lang="ts">
  ...
  const refreshHighestAndLowestPrice = async () => {
    confirmClick('', '', async () => {
      const r = await useOpsApi().refreshHoldPrice({ exchange: userStore.getUserInfo.defaultExchange });
      if (r.code == 0) {
        showToast('刷新成功');
      }
  ...
  const downloadHistoryCandles = async () => {};
