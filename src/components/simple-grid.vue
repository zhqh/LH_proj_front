<template>
  <div class="course-type">
    ...
    <nut-row>
      ...
      <nut-col :span="7">
        <nut-input placeholder="" v-model="customStrategyParam.param.position" />
      </nut-col>
      <nut-col :span="5">
      ...
      <nut-col :span="5">
        <div class="grid-count">
          <nut-input placeholder="" v-model="customStrategyParam.param.addition_count" />
          <div @click="showLowestOpenQuantity">
            <IconFont name="ask"></IconFont>
          </div>
        </div>
      </nut-col>
    </nut-row>
    ...
    <nut-row>
      ...
      <nut-col :span="7">
        <div class="dropdown-contain target-profit">
          <nut-input placeholder="" v-model="customStrategyParam.param.target_profit_rate" input-align="right" />%
        </div>
      </nut-col>
      ...
      <nut-col :span="7" v-show="customStrategyParam.param.target_profit_condition == 'gainThenFall'">
        <div class="dropdown-contain target-profit">
          <nut-input placeholder="" v-model="customStrategyParam.param.adjust_rate" input-align="right" />%
        </div>
      </nut-col>
    </nut-row>
    <nut-row>
  ...
  <div>
    <nut-popup position="bottom" v-model:visible="profitModeShow">
      <nut-picker v-model="profitMode" :columns="profitModeColumns" :title="$t('create_strategy.select_profit_mode')"
        @confirm="profitModePopupConfirm" @cancel="profitModeShow = false" />
    </nut-popup>
    <nut-popup position="bottom" v-model:visible="targetProfitConditionShow">
      <nut-picker v-model="targetProfitCondition" :columns="targetProfitConditionColumns"
        :title="$t('create_strategy.select_profit_condition')" @confirm="targetProfitConditionPopupConfirm"
        @cancel="targetProfitConditionShow = false" />
    </nut-popup>
    <nut-popup position="bottom" v-model:visible="lossConditionShow">
      <nut-picker v-model="lossCondition" :columns="lossConditionColumns"
        :title="$t('create_strategy.select_loss_condition')" @confirm="lossConditionPopupConfirm"
...
const verify = async (baseStrategyParam) => {
  ...
  if (baseStrategyParam.param.backtest != 'true') {
    if ((customStrategyParam.param.position / customStrategyParam.param.addition_count) * baseStrategyParam.param.lever < baseStrategyParam.context.lowestOpenQuantity) {
...
const showLowestOpenQuantity = async () => {
  ...
  showToast(`${globalProperties.$t('create_strategy.lowestOpenQuantity')}${baseStrategyParam.value.context.lowestOpenQuantity}${baseStrategyParam.value.param.unit}`);
}
</script>
